...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m56constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m56constructorimpl = Result.m56constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
        ...
        if (Result.m59exceptionOrNullimpl(m56constructorimpl) != null) {
            m56constructorimpl = ((Object) continuation.getClass().getName()) + '@' + getHexAddress(continuation);
        }
