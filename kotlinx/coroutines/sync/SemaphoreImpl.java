            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m1540constructorimpl = SegmentOrClosed.m1540constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1540constructorimpl = SegmentOrClosed.m1540constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1545isClosedimpl(m1540constructorimpl)) {
                break;
            }
            Segment m1543getSegmentimpl = SegmentOrClosed.m1543getSegmentimpl(m1540constructorimpl);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= m1543getSegmentimpl.getId()) {
                    break;
                } else if (!m1543getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    continue;
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m1543getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1543getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    m1543getSegmentimpl.remove();
                }
            }
            z = true;
            continue;
        } while (!z);
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1543getSegmentimpl(m1540constructorimpl);
    ...
    */
    private final boolean tryResumeNextFromQueue() {
            ...
            if (!SegmentOrClosed.m1545isClosedimpl(m1540constructorimpl)) {
                ...
                while (true) {
                    ...
                    if (segment.getId() >= m1543getSegmentimpl.getId()) {
                        ...
                        continue;
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, m1543getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m1543getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !semaphoreSegment4.acquirers.compareAndSet(i5, symbol3, symbol4);
        }
